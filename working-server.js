  ...
  return new Promise((resolve, reject) => {
    ...
    fs.createReadStream(filePath)
      ...
      .on('data', (data) => {
        ...
        if (contact.location) {
          potentialTags.push({ type: 'location', name: contact.location });
        }
        if (contact.company) {
          potentialTags.push({ type: 'company', name: contact.company });
        }
        if (contact.title) {
          potentialTags.push({ type: 'position', name: contact.title });
        }
        if (contact.industry) {
          potentialTags.push({ type: 'industry', name: contact.industry });
        }
