...
router.post('/import-and-enrich', async (req, res) => {
  try {
    ...
    // Step 3: Generate automated tags for each enriched contact
    const taggedContacts = [];
    for (const contact of matchedContacts) {
      try {
        const contactWithTags = await automatedTaggingService.generateAllTags(contact);
        taggedContacts.push(contactWithTags);
      } catch (taggingError) {
        logger.warn('Auto-tagging failed for contact', {
          contactName: contact.name,
          error: taggingError.message
        });
        taggedContacts.push(contact); // Keep contact without auto-tags
      }
    }
