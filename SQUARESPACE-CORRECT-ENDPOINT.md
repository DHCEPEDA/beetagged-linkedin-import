# Squarespace Widget Fix - Correct API Endpoint

## Problem Found
The widget was trying to use `/api/search-natural` endpoint, but your backend actually uses `/api/search`.

## ✅ Fixed
Updated `SQUARESPACE-FIXED-BACKEND.html` to use the correct endpoint: `/api/search`

## Updated Squarespace Code

Replace your Squarespace header code with this corrected version:
...
<script>
// IMPORTANT: Replace this URL with your actual Heroku app URL once MongoDB is configured
...
function updateStatus(status, message) {
    ...
    searchBtn.disabled = !isOnline;
}

async function checkServiceHealth() {
    try {
        updateStatus('connecting', '<span class="loading"></span> Checking service...');
        ...
        if (response.ok) {
            ...
            if (data.status && data.status.includes('BeeTagged')) {
                updateStatus('online', '✅ Service online - Ready to search contacts');
                showSampleContacts();
                return true;
            } else {
                updateStatus('offline', '⚠️ Service degraded - Limited functionality');
                showOfflineMessage();
                return false;
            }
        } else {
        ...
        updateStatus('offline', '❌ Service temporarily unavailable');
        showOfflineMessage();
        return false;
    }
}
...
function displayResults(data, query) {
    if (!data.contacts || data.contacts.length === 0) {
        searchResults.innerHTML = `
            <div class="beetagged-status status-offline">
                No contacts found for "${query}". Try a different search term.
            </div>
        `;
        return;
    }
    ...
    const contactsHtml = data.contacts.map(contact => `
        <div class="beetagged-contact">
            <div class="beetagged-contact-name">${escapeHtml(contact.name || 'Unknown')}</div>
            <div class="beetagged-contact-details">
                ${contact.company ? `<strong>Company:</strong> ${escapeHtml(contact.company)}<br>` : ''}
                ${contact.jobTitle ? `<strong>Title:</strong> ${escapeHtml(contact.jobTitle)}<br>` : ''}
                ${contact.location ? `<strong>Location:</strong> ${escapeHtml(contact.location)}<br>` : ''}
                ${contact.email ? `<strong>Email:</strong> ${escapeHtml(contact.email)}` : ''}
            </div>
        </div>
    `).join('');

    searchResults.innerHTML = `
        <h3>Found ${data.count} contact${data.count !== 1 ? 's' : ''} for "${escapeHtml(query)}"</h3>
        ${contactsHtml}
    `;
}
...
function showOfflineMessage() {
    searchResults.innerHTML = `
        <div class="beetagged-status status-offline">
            🔌 Service temporarily offline. Features limited until connection is restored.
            <br><small>This usually resolves automatically within a few minutes.</small>
        </div>
    `;
}
...
## Key Changes:
1. **Fixed API endpoint**: Changed from `/api/search-natural` to `/api/search`
2. **Health check**: Uses root endpoint `/` to check service status
3. **Error handling**: Better connection and error management

## Next Steps:
1. Replace your Squarespace header injection with this corrected code
2. The widget should now connect properly and show "✅ Service online"
3. For production, replace the BACKEND_URL with your Heroku app URL once MongoDB is configured

The widget will now connect to the correct endpoints and work properly.