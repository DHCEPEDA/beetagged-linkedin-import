// Download this file and use it as your index.js - it's your current file with just the CSV fixes
...
// FIXED: Storage variables declared once at the top
let contacts = [];
let tags = [];
...
// FIXED: Enhanced LinkedIn CSV parser
...
// Homepage
app.get('/', (req, res) => {
  const html = `<!DOCTYPE html><html><head><title>BeeTagged</title><style>body{font-family:Arial;margin:40px;background:#f8fafc}.container{max-width:800px;margin:0 auto;background:white;padding:30px;border-radius:10px}h1{color:#2563eb}.status{color:#059669;background:#dcfce7;padding:10px;border-radius:5px;margin:20px 0}.linkedin-section{background:#0077b5;color:white;padding:20px;border-radius:8px;margin:20px 0}.linkedin-section a{background:white;color:#0077b5;padding:10px 20px;text-decoration:none;border-radius:5px}</style></head><body><div class="container"><h1>🐝 BeeTagged</h1><div class="status">Server running - Contacts: ${contacts.length}</div><div class="linkedin-section"><h3>LinkedIn Import</h3><a href="/li-import">Import LinkedIn Connections</a></div><p><a href="/api/contacts">View Contacts API</a></p></div></body></html>`;
  res.send(html);
});

// LinkedIn import page
app.get('/li-import', (req, res) => {
  const html = `<!DOCTYPE html><html><head><title>LinkedIn Import</title><style>body{font-family:Arial;margin:40px;background:#f8fafc}.container{max-width:600px;margin:0 auto;background:white;padding:30px;border-radius:10px}h1{color:#0077b5}.upload-area{border:2px dashed #0077b5;padding:40px;text-align:center;border-radius:10px;margin:20px 0}button{background:#0077b5;color:white;padding:15px 30px;border:none;border-radius:5px;cursor:pointer}</style></head><body><div class="container"><h1>LinkedIn Import</h1><form action="/api/import/linkedin" method="post" enctype="multipart/form-data"><div class="upload-area"><h3>Upload LinkedIn CSV</h3><input type="file" name="linkedinCsv" accept=".csv" required><br><br><button type="submit">Import Contacts</button></div></form><a href="/">← Back</a></div></body></html>`;
  res.send(html);
});

// FIXED: LinkedIn import API with correct field name 'linkedinCsv'
