...
app.use(cors({
  origin: ['https://beetagged.com', 'https://www.beetagged.com', '*'],
...
app.get('/health', (req, res) => {
  res.json({ 
    ...
    service: 'BeeTagged Production Server',
    domain: req.hostname
  });
});
...
app.post('/api/search/natural', (req, res) => {
  try {
    ...
    contacts.forEach(contact => {
      ...
      // Role/title search
      if (normalizedQuery.includes('marketing') || normalizedQuery.includes('engineer') || normalizedQuery.includes('manager')) {
      ...
      searchTerms.forEach(term => {
        if (term.length > 2) {
          if (contact.name && contact.name.toLowerCase().includes(term)) {
            score += 50;
            matches = true;
          }
          ...
          if (contact.title && contact.title.toLowerCase().includes(term)) {
            score += 30;
            matches = true;
          }
        }
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`LinkedIn Import: /api/import/linkedin`);
