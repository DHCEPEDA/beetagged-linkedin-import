# Complete Squarespace Deployment Guide

## Problem: Backend Connection Issues

The widget shows "Service temporarily offline" because:
1. The Replit external URL doesn't work properly for external access
2. Your Heroku app needs to be deployed first

## ✅ Solution: Use Heroku Backend

### Step 1: Deploy to Heroku First

1. **Create Heroku app and deploy using the files I provided:**
   - `index.js` (copy from `SIMPLIFIED-BACKEND.js`)
   - `package.json` (copy from `WORKING-PACKAGE.json`)
   - `Procfile`
...
3. **Test your Heroku app:**
   ```bash
   curl https://your-app-name.herokuapp.com/
   ```

### Step 2: Update Squarespace Widget

Use this code in your Squarespace Header (replace `YOUR_HEROKU_APP` with your actual app name):
...
const BACKEND_URL = 'https://YOUR_HEROKU_APP.herokuapp.com';
...
async function searchContacts(query) {
    ...
    try {
        ...
        document.getElementById('searchResults').innerHTML = '<div class="beetagged-status status-offline">❌ Search temporarily unavailable. Please try again later.</div>';
    } finally {
...
function showOfflineMessage() {
    document.getElementById('searchResults').innerHTML = '<div class="beetagged-status status-offline">🔌 Service temporarily offline. Features limited until connection is restored.<br><small>This usually resolves automatically within a few minutes.</small></div>';
}
...
```javascript
const BACKEND_URL = 'https://your-real-app-name.herokuapp.com';
```

## Expected Result:

After deploying to Heroku and updating the URL, your widget will show:
- "✅ Service online - Ready to search contacts"
- Working search functionality
- Contact results from your MongoDB database

## Priority Order:

1. **Deploy Heroku first** (with MongoDB environment variable)
2. **Update Squarespace widget** with correct Heroku URL
3. **Test search functionality**

The key issue is that Replit external URLs don't work reliably for external access, so you need to use your Heroku deployment.